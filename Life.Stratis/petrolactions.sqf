...
petrol_refuel_tank = {
	...
	if (_liters_consumed > 0) then {
		...
		[player, 'money', -(_refuel_cost)] call inventory_add_item;
	};
};
...
petrol_set_refueling = {
	...
	[_vehicle, "refueling", _state] call object_setVariable;
...
petrol_get_refueling = {
	...
	_state = [_vehicle, "refueling"] call object_getVariable;
...
petrol_get_refuel_vehicle = {
	private["_vehicle"];
	_vehicle = [player, "refuel_vehicle"] call object_getVariable;
	if (undefined(_vehicle)) exitWith {null};
	_vehicle
};
...
petrol_get_refuel_action = {
	...
	_refuel_action = [_vehicle, "refuel_action"] call object_getVariable;;
...
petrol_set_refuel_action = {
	...
	[_vehicle, "refuel_action", _action] call object_setVariable;
	[player, "refuel_vehicle", _vehicle] call object_setVariable;
};
