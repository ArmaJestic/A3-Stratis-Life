...
_actions = [];
	...
	_action_id = _unit addaction [format["Activate %1", _smoke_name],A_actions_fnc_noscript,format['[_this select 1, "%1"] call A_halo_fnc_activate_smoke;', _smoke_class],1,false,true,"","true"];
	_actions = _actions + [[_action_id,_smoke_class]];
} foreach _smokes;

_action_id = _unit addaction [format["Deactivate A_other_fnc_smoke"],A_actions_fnc_noscript,format['[_this select 1] call A_halo_fnc_deactivate_smoke;'],1,false,true,"","true"];
