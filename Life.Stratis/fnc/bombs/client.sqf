...
while {true} do {
	...
	for [{_i=0}, {_i < (count A_bombs_var_localbombarray)}, {_i=_i+1}] do {
		_array    = (A_bombs_var_localbombarray select _i);
		_id = _array select 0;
		...
		if ( (vehicle player == _vehicle) and ((count _actionArr) == 0) ) then {
			...
			switch _item do {
				case "fernzuenderbombe": {
					_a1 = _v addAction [localize "STRS_inv_item_vehiclebomb_action_deactivate",  A_other_fnc_noscript, format["[""config"", ""defuse"", ""%1"", ""%2""] spawn A_SCRIPT_BOMBS;", _item, _id ]]; 
					_actionArr = _actionArr + [_a1];
				};
				case "zeitzuenderbombe": { 
					_a1 = _v addAction [localize "STRS_inv_item_vehiclebomb_action_deactivate", A_other_fnc_noscript,  format["[""config"", ""defuse"", ""%1"", ""%2""] spawn A_SCRIPT_BOMBS;", _item, _id ]];
					_a2 = _v addAction [localize "STRS_inv_item_vehiclebomb_action_changetimer", A_other_fnc_noscript, format["[""config"", ""Timer"",  ""%1"", ""%2""] spawn A_SCRIPT_BOMBS;", _item, _id ]];
		...
		(A_bombs_var_localbombarray select _i) set [3, _actionArr];
	};
