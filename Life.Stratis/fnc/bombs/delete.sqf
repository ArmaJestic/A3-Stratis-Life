...
for [{_i=0}, {_i < (count A_bombs_var_inv_serverbombarray)}, {_i=_i+1}] do {
	if (((A_bombs_var_inv_serverbombarray select _i) select 0) == _id) exitWith {
		A_bombs_var_inv_serverbombarray set [_i, ""];
		A_bombs_var_inv_serverbombarray = A_bombs_var_inv_serverbombarray - [""];
	};
};

for [{_i=0}, {_i < (count A_bombs_var_inv_localbombarray)}, {_i=_i+1}] do {
	if (((A_bombs_var_inv_localbombarray select _i) select 0) == _id) exitWith {
		if (alive(vehicle player)) then {
			_actionArr = ((A_bombs_var_inv_localbombarray select _i) select 3);
			for [{_j=0}, {_j < (count _actionArr)}, {_j=_j+1}] do {
				(vehicle player) removeAction (_actionArr select _j);
			};
		};

		A_bombs_var_inv_localbombarray set [_i, ""];
		A_bombs_var_inv_localbombarray = A_bombs_var_inv_localbombarray - [""];
	};
};
