...
// Firing Details
if (vehicle player != _unit) exitWith {null};
...
switch _M_effect do {
	...
	case "WP": {
		...
		[_pos_projectile] spawn A_effects_fnc_m_effect_wp;
	};
	case "IL": {	
		switch M_ill_style do {
			case "highest": {
				...
				[_projectile, M_ill_lit] spawn A_effects_fnc_m_effect_il;
			};
			case "timed": {
				sleep M_ill_delay;
				[_projectile, M_ill_lit] spawn A_effects_fnc_m_effect_il;
