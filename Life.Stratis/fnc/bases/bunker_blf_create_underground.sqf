// A_bases_fnc_bunker_blf_create_underground

#include "header.h"


params["_bunker_data","_offset","_heading"];

private _ref = createVehicle ["Sign_Sphere10cm_F", [0,0,0], [], 0, "NONE"];
[_ref, object_attach_reference,  _offset, _heading, false] call A_object_fnc_fake_attach;
[[_ref, true], "A_object_fnc_remote_hideObject", _ref] call A_jip_fnc_register;
...
private _doors = [blue4_bunker_door_l_u,blue4_bunker_door_m_u,blue4_bunker_door_r_u];
{
	private _door_data = [];
	_door_data set [BUNKER_INDEX_DOOR_ACTION_LABEL, format["Exit %1", (_bunker_data select BUNKER_INDEX_DATA_NAME)]];
	_door_data set [BUNKER_INDEX_DOOR_ACTION_CALLBACK, "A_bases_fnc_bunker_exit"];
