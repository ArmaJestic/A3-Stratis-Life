...
ARGV(0,_player);
if (!([_player] call A_player_fnc_blufor)) exitWith {null};
...
if ([player, "stun_light_on"] call A_inventory_fnc_get_item_amount > 0) then {
	[_player, "A_item_fnc_stun_armor", "light", true] call A_object_fnc_setVariable;
}else{ if ([player, "stun_full_on"] call A_inventory_fnc_get_item_amount > 0) then {
	[_player, "A_item_fnc_stun_armor", "full", true] call A_object_fnc_setVariable;
}else{
