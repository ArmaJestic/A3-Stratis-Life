...
bank_menu_dialog = { _this spawn {
	...
	if (recently_robbed_bank)  exitWith {
	...
	(
		...
		'	[' + str(_player) + ', (missionNamespace getVariable _variable_name), _amount] call interact_deposit;'+
		'closedialog 0;'
	)];
	...
	(
		'	private["_amount"];' +
		'	_amount = ([ctrlText bank_menu_amount_field_idc] call parse_number);' +
		'	[' + str(_player) + ', _amount] call interact_withdraw;'+
		'closedialog 0;'
	)];
