...
if (not(undefined(arrest_menu_functions_defined))) exitWith {};



arrest_menu_time_slider_changed = {
	ARGV(0,_control);
...
arrest_menu_bail_slider_changed = {
	ARGV(0,_control);
...
arrest_menu_setup = {
	...
	_arrest_menu_time_slider ctrlAddEventHandler ["SliderPosChanged", "(_this) call arrest_menu_time_slider_changed"];
	_arrest_menu_time_slider sliderSetRange _time_range;
	_arrest_menu_time_slider sliderSetPosition _time_default; 
	[_arrest_menu_time_slider] call arrest_menu_time_slider_changed;
	
	_arrest_menu_bail_slider ctrlAddEventHandler ["SliderPosChanged", "(_this) call arrest_menu_bail_slider_changed"];
	_arrest_menu_bail_slider sliderSetRange _bail_range;
	_arrest_menu_bail_slider sliderSetPosition _bail_default; 
	[_arrest_menu_bail_slider] call arrest_menu_bail_slider_changed;
...
arrest_menu_create = {
	ARGV(0,_player);
	ARGV(1,_target);
	[toUpper("Arrest Menu"), [1,25], 5, [1,100], 1] call arrest_menu_setup;
	
	buttonSetAction [arrest_menu_submit_button_idc, '[' + str(_player) + ',' + str(_target) + ', ([(sliderPosition arrest_menu_time_slider_idc)] call parse_number), ([(sliderPosition arrest_menu_bail_slider_idc)] call parse_number)] call interact_arrest_player;'];
};

arrest_menu_functions_defined = true;