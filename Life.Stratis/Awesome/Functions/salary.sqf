...
if (not(undefined(salary_functions_defined))) exitWith {null};


cop_salary_handout = {
	if (not(iscop)) exitWith {null};
	private["_player"];
	_player = player;
	...
	if ("patrol" call inventory_has_license) then {
		_income = _income + 500;
	};

	if ("response" call inventory_has_license) then {
		_income = _income + 1000;
	};

	if ("sobr" call inventory_has_license) then {
		_income = _income + 1250;
	};

	if ("air_support_training" call inventory_has_license) then {
		_income = _income + 1000;
	};

	if ([_player] call player_chief) then {
	...
	[_player, (round _income)] call bank_transaction;
...
civilian_salary_handout = {
	...
	if ([_player] call player_president) then {
		...
		(format["if ([player] call player_president) then {MayorTaxes = MayorTaxes + %1;};", _taxes]) call broadcast;
	};};
