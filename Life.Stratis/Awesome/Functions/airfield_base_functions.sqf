...
if (not(undefined(airfield_base_functions_defined))) exitWith {};
...
if (isClient) then {
	waitUntil {(not(isNil "airfield_base_setup_complete") && {airfield_base_setup_complete})};
	respawn_west_proxy setVariable ["respawn", true];
	respawn_west_proxy setVariable ["radius", 5];
};

airfield_base_functions_defined = true;