...
if (not(undefined(inventory_menu_functions_defined))) exitWith {};

interact_inventory_menu_item_changed = {
	ARGV(0,_control);
	ARGV(1,_index);
	...
	ctrlSetText [inventory_desciption_text_idc, format ["%1", _item call inventory_get_item_description_2]];
...
inventory_menu_setup = {
	...
	_info_field ctrlAddEventHandler ["SetFocus", "(_this) call inventory_menu_combo_focus"];
	_info_field ctrlAddEventHandler ["KillFocus", "(_this) call inventory_menu_combo_blur"];
	_info_field ctrlAddEventHandler ["LBSelChanged", "(_this) call interact_inventory_menu_item_changed"];
	
	buttonSetAction [inventory_button_use_idc, format["[player, lbData [1, (lbCurSel 1)], ([(ctrlText 501)] call parse_number)] call interact_item_use_inventory; closedialog 0;"]];
	buttonSetAction [inventory_button_drop_idc, format["[player, lbData [1, (lbCurSel 1)], ([(ctrlText 501)] call parse_number)] call interact_item_take_inventory; closedialog 0;"]];
	buttonSetAction [inventory_button_give_idc, format["[player, lbData [1, (lbCurSel 1)], ([(ctrlText 501)] call parse_number), (missionNamespace getVariable (lbData [99, (lbCurSel 99)]))] call interact_item_give; closedialog 0;"]];
...
inventory_menu_functions_defined = true;