...
if (not(undefined(marker_functions_defined))) exitWith {null};
...
marker_loop_draw = {
	if (not([player] call camera_get_map_open)) exitWith {null};
...
marker_side_loop_draw = {
	...
	_has_admin_camera = [_player_variable, "has_admin_camera"] call object_getVariable;
	...
	_under = [_player_variable] call under_base;
