...
if (_art == "ausrauben") then {
	...
	if(!(call iventory_is_armed) and !debug)exitWith{player groupChat localize "STRS_bank_rob_noweapon";};
	robenable = false;
	...
	if (alive player) then {
		[player, 'money', local_cash] call inventory_add_item;
	...
	if(_rblock != rblock)exitWith {null};
...
if (_art == "opfer") then {
	...
	_bank_account = [player] call bank_get_value;
	_insurances_inv = [player, 'bankinsurance'] call inventory_get_item_amount;
	_insurances_stor = [player, 'bankinsurance', 'private_storage'] call inventory_get_storage_amount;
	...
	if (_insurances_inv > 0) exitWith {
		...
		[player, 'bankinsurance', -(1)] call inventory_add_item;
	};
	
	if(_insurances_stor > 0) exitWith {
		player groupChat localize "STRS_bank_rob_lostnomoney";
		[player, 'bankinsurance', -(1), 'private_storage'] call inventory_storage_add_item;
	};
};
